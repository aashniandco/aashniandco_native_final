...
//   bool _hasImmediateDeliveryOption = false;
//
...
//
//     // 2. Add Listener to detect scroll direction
//     _scrollController.addListener(() {
//       if (_scrollController.position.userScrollDirection == ScrollDirection.reverse) {
//         // User is scrolling down (swiping up) -> Hide Bar
...
//         // User is scrolling up (swiping down) -> Show Bar
//         if (!_isBottomBarVisible) {
//           setState(() {
...
//       final List<dynamic> sizeOptions = fullProductData['processed_size_options'] ?? [];
//
//       // Check for 'enquire_1' flag
//       final List<dynamic> enquireFlag = fullProductData['enquire_1'] ?? [];
//       final bool shouldEnquire = enquireFlag.contains(1);
//
//       if (mounted) {
//         setState(() {
//           _fetchedProductDesc = fullDesc;
...
//           _sizeList = sizeOptions.map((size) => size['label'].toString()).toList();
//           _areDetailsLoading = false;
//           _showEnquireButton = shouldEnquire; // Update the new state variable
//         });
//       }
//     } catch (e) {
//       print("Error fetching full product details for SKU $sku: $e");
...
//   // In _ProductDetailNewInDetailScreenState
//
//   // --- CUSTOM RECTANGULAR SIZE BUTTONS ---
...
//                 child: const Text("Size Guide", style: TextStyle(decoration: TextDecoration.underline, fontSize: 13)),
//               ),
//             ],
//           ),
//           const SizedBox(height: 15),
//           if (_areDetailsLoading)
//             const Center(child: LinearProgressIndicator(color: Colors.black, backgroundColor: Colors.white))
//           else
//             Wrap(
//               spacing: 10,
//               runSpacing: 10,
//               children: sizeOptions.keys.map((displayKey) {
//                 int index = sizeOptions.keys.toList().indexOf(displayKey);
...
//           elevation: 0,
//         ),
//         child: Text(text.toUpperCase(), style: const TextStyle(color: Colors.white, letterSpacing: 1)),
//       ),
//     );
//   }
//
...
//         backgroundColor: Colors.black, // High-end apps use black/white badges
...
//   }
//
//   Widget _buildImageSection(List fallbackImages) {
//     // Determine which images to show
//     final imagesToShow = _productImages.isNotEmpty ? _productImages : fallbackImages;
//
//     return Column(
//       children: [
//         SizedBox(
//           height: 550, // Standard height for luxury product shots
//           width: double.infinity,
...
//               final imageUrl = imagesToShow[index] is ProductImage
//                   ? imagesToShow[index].imageUrl
//                   : imagesToShow[index];
//
//               return GestureDetector(
...
//                   child: Image.network(
//                     imageUrl.toString(),
//                     fit: BoxFit.cover, // Ensures the image fills the area elegantly
...
//         if (!_areImagesLoading && imagesToShow.length > 1)
//           Padding(
...
//   Widget _buildExpandableSection(String title, dynamic content) {
//     String textContent = (content is List) ? content.join(", ") : content.toString();
//     return Container(
//       decoration: BoxDecoration(border: Border(bottom: BorderSide(color: Colors.grey[200]!))),
//       child: ExpansionTile(
//         title: Text(title, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600, letterSpacing: 1)),
//         iconColor: Colors.black,
//         collapsedIconColor: Colors.black,
//         childrenPadding: const EdgeInsets.fromLTRB(20, 0, 20, 20),
//         children: [
//           Text(textContent, style: TextStyle(color: Colors.grey[700], height: 1.5, fontSize: 14)),
//         ],
//       ),
//     );
//   }
...
//
//     String displaySymbol = '£'; // Styled for the luxury look in video
...
//         ),
//         centerTitle: true,
//         title: Image.asset('assets/logo.jpeg', height: 25), // Smaller, cleaner logo
//         actions: [
//           IconButton(
...
//           children: [
//             // 1. IMAGE CAROUSEL
//             _buildImageSection(fallbackImages),
//
//             // 2. PRODUCT HEADER
//             Padding(
...
//                 children: [
//                   Text(
//                     (widget.product['designer_name'] ?? "DESIGNER").toString().toUpperCase(),
...
//             ),
//
//             // 3. SIZE SELECTION (Rectangular Grid)
//             if (!_showEnquireButton) _buildSizeSelectionSection(),
//
...
//               child: Column(
//                 children: [
//                   if (_showEnquireButton)
//                     _buildPrimaryButton("ENQUIRE NOW", () => _showEnquiryDialog(context, widget.product, _apiCountries))
//                   else ...[
//                     _buildPrimaryButton("Add to Bag", onAddToCartPressed),
//                     const SizedBox(height: 12),
//                     _buildSecondaryButton(
//                       _isAddingToWishlist ? "Adding..." : "Add to Wish List",
//                       Icons.favorite_border,
//                       _onAddToWishlistPressed,
//                     ),
//                   ],
//                 ],
//               ),
...
//                     Expanded(
//                       child: Text(
//                         "Estimated delivery: ${widget.product['child_delivery_time'] ?? 'Standard'}",
...
//               ),
//
//             // 6. EXPANDABLE SECTIONS (Editor's Notes Style)
//             _buildExpandableSection("EDITOR'S NOTES", _fetchedProductDesc.isNotEmpty ? _fetchedProductDesc : widget.product['prod_desc']),
//             _buildExpandableSection("SIZE & FIT", "Fits true to size. Take your normal size. Model is 175cm/ 5'9\" and is wearing a UK 8."),
//             _buildExpandableSection("DETAILS & CARE", widget.product['disclaimer'] ?? "Professional dry clean only."),
//
//             const SizedBox(height: 40),
//
//             // 7. DISCOVER MORE
//             const Center(
//               child: Text("YOU MAY ALSO LIKE", style: TextStyle(letterSpacing: 2, fontWeight: FontWeight.bold)),
//             ),
//             const SizedBox(height: 20),
...
// //                       //       : PageView.builder(
// //                       //     controller: PageController(viewportFraction: 0.90), // ★ Show partial next image
// //                       //     physics: _currentScale > 1.0
// //                       //         ? const NeverScrollableScrollPhysics()
// //                       //         : const PageScrollPhysics(),
// //                       //     onPageChanged: (index) {
...
// //                       //         scale: index == _currentImageIndex ? 1.0 : 0.92, // ★ Scale effect
...
// //                       //
// //                       //           child: ClipRRect(
// //                       //             borderRadius: BorderRadius.circular(16), // ★ Rounded view like Air India
// //                       //
...
// //                             ? const Center(child: CircularProgressIndicator())
// //                             : PageView.builder(
// //                           controller: _pageController, // full width now
// //                           physics: _currentScale > 1.0
// //                               ? const NeverScrollableScrollPhysics()
// //                               : const PageScrollPhysics(),
// //
...
// //                                   fit: BoxFit.cover, // ★ covers full width/height
// //                                   errorBuilder: (c, e, s) => Container(
...
// //                           (_productImages.isNotEmpty || fallbackImages.isNotEmpty))
// //                         Padding(
...
// // // // --- Image Section (Horizontal Carousel) ---
// // //                     SizedBox(
// // //                       height: 600, // Fixed height for the image area
// // //                       width: double.infinity,
// // //                       child: _areImagesLoading
// // //                           ? const Center(child: CircularProgressIndicator())
// // //                           : PageView.builder(
...
// // //                     ),
// // //
// // // // --- Dots are now BELOW the SizedBox, not inside a Stack ---
// // //                     if (!_areImagesLoading &&
// // //                         (_productImages.isNotEmpty || fallbackImages.isNotEmpty))
// // //                       Padding(
// // //                         padding: const EdgeInsets.only(top: 15.0, bottom: 10.0), // Space between image and dots
// // //                         child: Row(
// // //                           mainAxisAlignment: MainAxisAlignment.center,
// // //                           children: List.generate(
// // //                             _productImages.isNotEmpty
...
// // //                               return AnimatedContainer(
// // //                                 duration: const Duration(milliseconds: 300),
// // //                                 width: _currentImageIndex == index ? 10.0 : 8.0, // Optional: Make active dot slightly larger
// // //                                 height: _currentImageIndex == index ? 10.0 : 8.0,
// // //                                 margin: const EdgeInsets.symmetric(horizontal: 4.0),
// // //                                 decoration: BoxDecoration(
// // //                                   shape: BoxShape.circle,
// // //                                   // Active dot is Black, Inactive is lighter Grey
// // //                                   color: _currentImageIndex == index
// // //                                       ? Colors.black
// // //                                       : Colors.grey.shade300,
// // //                                 ),
...
// // //                       ),
// //
// // // ... rest of your code (Product details, etc.) ...
// //
// //                       // SizedBox(
// //                       //   height: 500, // Adjusted height for better vertical coverage
// //                       //   width: double.infinity,
// //                       //   child: Stack(
...
// //                       //                 imageUrl,
// //                       //                 width: double.infinity,
// //                       //                 fit: BoxFit.cover, // Changed to cover for full carousel effect
// //                       //                 errorBuilder: (c, e, s) => Container(
// //                       //                   color: Colors.grey[300],
// //                       //                   alignment: Alignment.center,
// //                       //                   child: const Icon(
...
// //                       //                   decoration: BoxDecoration(
// //                       //                     shape: BoxShape.circle,
// //                       //                     // Active dot is Black, Inactive is Grey
// //                       //                     color: _currentImageIndex == index
// //                       //                         ? Colors.black
...
// //                             if (_showEnquireButton)
// //                               ElevatedButton(
// //                                 onPressed: () => _showEnquiryDialog(context, widget.product,_apiCountries),
// //                                 style: ElevatedButton.styleFrom(
...
// //                                 onPressed: _onAddToWishlistPressed,
// //                               ),
// //                             ],
// //                           ),
// //                         ),
...
// //                                       ? const Center(child: CircularProgressIndicator())
// //                                       : _buildProductCarousel(_youMayAlsoLikeProducts,isMoreFromDesignerTab: false),
// //                                   _isLoadingDesigner
// //                                       ? const Center(child: CircularProgressIndicator())
// //                                       : _buildProductCarousel(_designerProducts,isMoreFromDesignerTab: true),
// //                                 ],
// //                               ),
// //                             ),
// //                           ],
...
// //
// //               // --- Bottom support bar ---
// //               // --- Animated Bottom Support Bar ---
// //               //         AnimatedContainer(
// //               //         duration: const Duration(milliseconds: 300), // Animation speed
// //               //         height: _isBottomBarVisible ? 120.0 : 0.0,   // 👈 2. HIDE/SHOW LOGIC
// //               //         curve: Curves.easeInOut,
// //               //         child: SingleChildScrollView( // Prevents overflow error when shrinking
// //               //         physics: const NeverScrollableScrollPhysics(),
// //               //         child: Container(
...
// //               // children: [
// //               // _supportButton(FontAwesomeIcons.whatsapp, "Chat With Us",
...
// //               // () => _sendEmail("customercare@aashniandco.com")),
// //               // ],
// //               // ),
// //               // ],
// //               // ),
...
// //             duration: const Duration(milliseconds: 300),
// //             curve: Curves.easeInOut,
// //             // 👈 3. MOVE OFF SCREEN IF HIDDEN
// //             bottom: _isBottomBarVisible ? 90 : -100,
// //             right: 20,
// //             child: GestureDetector(
// //               onTap: () {
// //                 _openWhatsApp("+918375036648");
...
//   }
//
//   // void _showEnquiryDialog(BuildContext context, Map<String, dynamic> product, List<Country> _apiCountries) {
...
//   //             "product_name": product['designer_name'] ?? "Unknown Product"
//   //           });
//   //
//   //           try {
...
//   //
//   //         return AlertDialog(
//   //             title: Text('Enquire about ${product['designer_name'] ?? "this product"}'),
...
//                   Text(
//                     product['designer_name'] ?? "This Product",
//                     style: TextStyle(
...
//   // In _ProductDetailNewInDetailScreenState
//   //
//   Widget _buildProductCarousel(List<Map<String, dynamic>> products, {bool isMoreFromDesignerTab = false}) {
//     final currencyState = context.watch<CurrencyBloc>().state;
//     String displaySymbol = '₹';
...
//                             if (isMoreFromDesignerTab && _showEnquireButton)
//                               Center(
//                                 child: ElevatedButton(
//                                   onPressed: () => _showEnquiryDialog(context, product,_apiCountries),
//                                   style: ElevatedButton.styleFrom(
//                                     backgroundColor: Colors.black,
//                                     foregroundColor: Colors.white,
...
// In _ProductDetailNewInDetailScreenState
  ...
  void _shareToWhatsApp() async {
    final String designer = widget.product['designer_name'] ?? "Designer";
    final String desc = widget.product['short_desc'] ?? "";
    final String sku = widget.product['prod_sku'] ?? "";

    // Construct a URL to the product.
    // If your API doesn't provide a direct link, we use a search link as a fallback
    final String productUrl = "https://aashniandco.com/catalogsearch/result/?q=$sku";

    // Construct the message
    final String message = "Hello Aashni & Co, I'm interested in this product:\n\n"
        "*Designer:* $designer\n"
        "*Product:* $desc\n"
        "*SKU:* $sku\n"
        "*Link:* $productUrl";
    //
    // final String phone = "+918375036648";
    final String phone = "+918375036648";
    final String encodedMsg = Uri.encodeComponent(message);

    // Using https://wa.me/ for better cross-platform compatibility
...
// Function to handle Native Sharing
  void _shareProductNative() {
    final String designer = widget.product['designer_name'] ?? "Designer";
    final String desc = widget.product['short_desc'] ?? "";
    final String sku = widget.product['prod_sku'] ?? "";
    final String productUrl = "https://aashniandco.com/catalogsearch/result/?q=$sku";

    final String shareText = "Check out this beautiful piece by $designer on Aashni & Co!\n\n"
        "$desc\n\n"
        "View here: $productUrl";

    Share.share(shareText, subject: 'Interested in $designer');
  }
...
// ... rest of your code (Product details, etc.) ...
                    ...
                    Padding(
                      padding: const EdgeInsets.all(16.0),
                      child: Text(
                        "Ships In: ${(widget.product['child_delivery_time'] is List) ? widget.product['child_delivery_time'].join(", ") : widget.product['child_delivery_time'] ?? "Not specified"}",
                        style: const TextStyle(fontSize: 14),
                      ),
                    ),
                    // Padding(
                        ...
                        children: [
                          ...
                          if (isMoreFromDesignerTab && _showEnquireButton)
                            ...
                            Center(
                              child:
                              Text(
                                  priceFormatter.format(convertedPrice),
